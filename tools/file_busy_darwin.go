...
func IsFileBusyByInterval(filename string, busyInterval int64) (bool, *vo.FileInfo) {
	...
	fileInfo.AccessTimestamp = time.Unix(int64(fileData.Atimespec.Sec), int64(fileData.Atimespec.Nsec)).UnixNano() / 1e6
	fileInfo.CreateTimestamp = time.Unix(int64(fileData.Ctimespec.Sec), int64(fileData.Ctimespec.Nsec)).UnixNano() / 1e6
	fileInfo.ModifyTimestamp = time.Unix(int64(fileData.Mtimespec.Sec), int64(fileData.Mtimespec.Nsec)).UnixNano() / 1e6
